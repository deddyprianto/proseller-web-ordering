...
const calculateTAX = async (products,returnData,event,listAllProducts=[]) => {
    let surchargeData = {
        "type": "FixAmount",
        "value": 0
    };

    if(typeof returnData.orderingMode !== 'undefined' && typeof returnData.outlet.surcharge !== 'undefined'){
    ...
    let setPercentage = (amount, discountableAmount) => {
        if (parseFloat(discountableAmount) === 0)
            return "0%";
    ...
    let setNettAmount = (originalAmount, minValue, roundType) => {
        ...
        let nearestValue = 0;
        if (downDiff < upDiff)
            nearestValue = downValue;
        else
            nearestValue = upValue;

        if (roundType === 'Down')
            roundedAmount = downValue;
        else if (roundType === 'Up')
            roundedAmount = upValue;
        else
            roundedAmount = nearestValue;
    ...
    for (let i = 0; i < products.length; i++) {
        ...
        products[i]['unitPrice'] = (typeof products[i]['unitPrice'] !== "undefined") ? products[i]['unitPrice'] : parseFloat(products[i].product['retailPrice']);
        products[i]['grossAmount'] = products[i]['grossAmount'] || parseFloat(products[i].quantity) * parseFloat(products[i]['unitPrice']);
        ...
        if ((products[i].product['isNonDiscountable'] === false)) {
            ...
            if(typeof products[i]['lineDiscPercentage'] !== "number"){
                products[i]['lineDiscPercentage'] = (typeof products[i]['lineDiscPercentage'] !== "undefined")
                    ? products[i]['lineDiscPercentage'] :
                    setPercentage(products[i]['lineDiscAmount'],
                        products[i]['discountableAmount']);
            }
            if(typeof products[i]['lineDiscAmount'] !== "number"){
                products[i]['lineDiscAmount'] = (typeof products[i]['lineDiscAmount'] !== "undefined")
                    ? products[i]['lineDiscAmount'] :
                    setAmount(products[i]['lineDiscPercentage'], products[i]['discountableAmount']);
            }
            if(typeof products[i]['billDiscAmount'] !== "number"){
                products[i]['billDiscAmount'] = (typeof products[i]['billDiscAmount'] !== "undefined") ? products[i]['billDiscAmount'] : 0;
            }
        } else {
        ...
        if(typeof products[i]['totalDiscAmount'] !== "number"){
            products[i]['totalDiscAmount'] = parseFloat(products[i]['lineDiscAmount']) + parseFloat(products[i]['billDiscAmount']);
        }
        if(typeof products[i]['amountAfterDisc'] !== "number"){
            products[i]['amountAfterDisc'] = parseFloat(products[i]['grossAmount']) - parseFloat(products[i]['totalDiscAmount']);
        }

        // 3. calculate tax
        if(typeof products[i]['taxPercentage'] !== 'number'){
            if(products[i].product['taxRuleID'] === "NO-TAX" || typeof products[i].product['taxRuleID'] === 'undefined'){
                products[i]['taxPercentage'] = 0;
            } else {
                products[i]['taxPercentage'] = (typeof returnData['outlet']['taxPercentage'] === 'undefined')
                    ? 0 : parseFloat(returnData['outlet']['taxPercentage']);
            }
        }

        if(typeof products[i]['amountBeforeSurcharge'] !== "number"){
            products[i]['amountBeforeSurcharge'] = (products[i].product['taxRuleID'] === "INC-TAX")
                ? setAmountBeforeSurcharge(products[i]['amountAfterDisc'], products[i]['taxPercentage'])
                : products[i]['amountAfterDisc'];
        }
        if(typeof products[i]['surchargeAmount'] !== "number"){
            products[i]['surchargeAmount'] = setSurchargeAmount(products[i]['amountBeforeSurcharge']);
        }
        if(typeof products[i]['taxableAmount'] !== 'number') {
            products[i]['taxableAmount'] = products[i]['surchargeAmount'] + products[i]['amountBeforeSurcharge'];
        }

        if(typeof products[i]['taxAmount'] !== "number"){
            products[i]['taxAmount'] = setTaxAmount(products[i]['taxableAmount'], products[i]['taxPercentage']);
        }

        if(typeof products[i]['amountAfterTax'] !== "number"){
            products[i]['amountAfterTax'] = parseFloat(products[i]['taxableAmount']) + parseFloat(products[i]['taxAmount']);
        }

        // 4. set rounding value
        if(typeof products[i]['roundingAmount'] !== "undefined"){
            products[i]['roundingAmount'] = (typeof returnData.outlet['roundingAmount'] === 'undefined')
                ? 0 : parseFloat(returnData.outlet['roundingAmount']);
        }
        if(typeof products[i]['nettAmount'] !== "number"){
            products[i]['nettAmount'] = (typeof returnData.outlet['roundingRule'] === 'undefined' || returnData.outlet['roundingRule'] === "None")
                ? products[i]['amountAfterTax']
                : setNettAmount(products[i]['amountAfterTax'],
                    products[i]['roundingAmount'],
                    returnData.outlet['roundingRule']
                );
        }
        products[i]['modifiers'] = (typeof products[i]['modifiers'] !== "undefined") ? products[i]['modifiers'] : [];
                ...
                if (modifiers.modifier && modifiers.modifier.details) {
                    for (let k = 0; k < modifiers.modifier.details.length; k++) {
                        ...
                        if(typeof details.productID !== 'undefined'){
                            details.productID = details.productID.startsWith('product::') ? details.productID : `product::${details.productID}`;
                        }

                        // try to get product details from product service
                        let productResult = _.find(listAllProducts,{'sortKey':details.productID});

                        if (typeof productResult !== 'undefined') {
                            let productField = ["id", "name", "categoryID", "categoryName", "defaultImageURL", "retailPrice", "taxRuleID", "isNonDiscountable", "barcode"];
                            let productTemp = {};
        ...
        for (let j = 0; j < totalField.length; j++) {
            ...
            if(typeof products[i][totalField[j].unit] !== "undefined"){
                returnData[totalField[j].total] += parseFloat(products[i][totalField[j].unit]);
            }
        }
    ...
    if(typeof returnData.details !== 'undefined'){
        if(returnData.details.length > 0){
            for(let i = 0; i < returnData.details.length; i++){
                let detailsDecimalFields = Object.getOwnPropertyNames(returnData.details[i]);
                for(let field of detailsDecimalFields){
                    if(typeof returnData.details[i][field] === 'number'){
                        returnData.details[i][field] = roundToTwo(returnData.details[i][field]);
                    }
                }
            }
        }
