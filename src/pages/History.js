...
class History extends Component {
  ...
  componentDidMount = async () => {
    ...
    if (this.state.dataPending.length > 0) this.setState({ isTransaction: false })
  ...
  render() {
    let { countryCode, dataPendingLength, dataPending, isTransaction, loadingShow } = this.state
        ...
        <div id="primary" className="content-area">
          <div className="stretch-full-width">
            <div style={{ flexDirection: "row", position: "fixed", zIndex: 10, width: "100%", marginTop: -60 }}>
              <Button className={isTransaction ? "use-select" : "un-select"}
                style={{ height: 50, color: "#FFF", fontWeight: "bold" }}
                onClick={() => this.setState({ isTransaction: true })}>Orders</Button>
              <Button className={!isTransaction ? "use-select" : "un-select"}
                style={{ height: 50, color: "#FFF", fontWeight: "bold" }}
                onClick={() => this.setState({ isTransaction: false })}>{`Pending Orders ${dataPendingLength > 0 ? `(${dataPendingLength})` : ''}`}</Button>
            </div>
            <main id="main" className="site-main" style={{ textAlign: "center", paddingBottom: 40 }}>
              <div style={{ marginTop: 20 }}>
                {isTransaction && <HistoryTransaction countryCode={countryCode} />}
                {
                  !isTransaction &&
                  <HistoryPending
