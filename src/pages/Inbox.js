...
const ModalDetailInbox = loadable(() => import('../components/inbox/ModalDetailInbox'))
...
class Inbox extends Component {
  ...
  fetchMoreData = async () => {
    ...
    let response = await this.props.dispatch(InboxAction.getBroadcast({ take: 14, skip: broadcast.length }));
  ...
  handleDetail = async (detailInbox) => {
    if (!detailInbox.isRead) {
      ...
      await this.props.dispatch(InboxAction.setData({ Data: data }, CONSTANT.KEY_GET_BROADCAST));
    } else {
      this.setState({ detailInbox })
    }
  }
  ...
  render() {
    ...
    return (
      <div className="col-full" style={{ marginTop: 70, marginBottom: 50 }}>
