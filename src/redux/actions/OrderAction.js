...
function shareURL(tableNo, outletID, orderMode) {
  ...
  return async dispatch => {
    ...
    const response = await OrderingService.api('POST', payload, `cart/generateshareurl/`, 'Bearer');
...
function processAddCart(defaultOutlet, selectedItem) {
  return async dispatch => {
    ...
    if (selectedItem.remark != "" || selectedItem.remark != undefined) product.remark = selectedItem.remark;

    // IF MODIFIER EXIST
    if (!isEmptyArray(selectedItem.product.productModifiers)) {
      const productModifierClone = JSON.stringify(selectedItem.product.productModifiers);
      let productModifiers = JSON.parse(productModifierClone);

      productModifiers = productModifiers.filter(item => item.postToServer == true);
      ...
      for (let i = 0; i < product.modifiers.length; i++) {
        ...
        for (let j = 0; j < data.modifier.details.length; j++) {
          if (data.modifier.details[j].quantity != undefined && data.modifier.details[j].quantity > 0) {
...
function processUpdateCart(basket, product) {
  return async dispatch => {
    ...
    const find = basket.details.find(data => data.product.id == product.product.id);
    ...
    if (product.remark != "" && product.remark != undefined) dataproduct.remark = product.remark;
...
function updateCart(payload) {
  return async dispatch => {
    const response = await OrderingService.api('POST', payload, `cart/updateitem`, 'Bearer');
    if (response.ResultCode === 400) await dispatch(AuthActions.refreshToken());
...
function getCart(isSetData = true) {
  return async dispatch => {
    ...
    try { document.getElementById("close-modal").click(); } catch (error) { }
    if (response.ResultCode === 400) await dispatch(AuthActions.refreshToken())
...
function submitTakeAway(payload) {
  return async (dispatch) => {
    let response = await OrderingService.api('POST', payload, `cart/submitTakeAway`, 'Bearer')
    if (response.resultCode === 400) await dispatch(AuthActions.refreshToken())
    return response
  };
}
...
function getProvider() {
  return async (dispatch) => {
    let response = await OrderingService.api('POST', null, `delivery/providers`, 'Bearer')
    if (response.resultCode === 400) await dispatch(AuthActions.refreshToken())
    return response.data
  };
}
...
function getCartCompleted(id) {
  return async (dispatch) => {
    let response = await OrderingService.api('GET', null, `outlet/cart/getcompleted/${id}`, 'Bearer')
    if (response.resultCode === 400) await dispatch(AuthActions.refreshToken())
    return response
  };
}
...
function cartUpdate(payload) {
  return async (dispatch) => {
    let response = await OrderingService.api('POST', payload, `outlet/cart/update`, 'Bearer')
    if (response.resultCode === 400) await dispatch(AuthActions.refreshToken())
    return response
  };
}
...
function changeOrderingMode(payload) {
  return async (dispatch) => {
    let response = await OrderingService.api('POST', payload, `cart/changeOrderingMode`, 'Bearer')
    if (response.resultCode === 400) await dispatch(AuthActions.refreshToken())
    return response
  };
}
