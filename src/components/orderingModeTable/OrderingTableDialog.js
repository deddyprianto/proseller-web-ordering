...
const OrderingTableDialog = ({ open, onClose, dataMerchant, colorState }) => {
  ...
  //         },
  //       ];
  //       //TODO: Please remove the functi on after update from backend
  ...
  const RenderRandomTable = () => {
    return dataMerchant.numberTable.map((item) => {
      return (
        <div
          style={{
            display: 'flex',
            ...
            alignItems: 'center',
            width: '80px',
            border: `1px solid ${colorState.primary}90`,
            borderRadius: '10px',
            padding: '10px 15px',
            color: colorState.primary,
          }}
  ...
  const HandleLabelModal = () => {
    if (dataMerchant.tableNumber.sequencing === 'IN-ORDER') {
      return (
        <Typography
  ...
  return (
    ...
    >
      <LoadingOverlayCustom active={isLoading} spinner text='Loading...'>
        ...
        {dataMerchant.tableNumber.sequencing === 'IN_ORDER' ? (
          <RenderInOrderTable />
        ) : (
          <Slider {...settings}>
            <div
