...
class ModalProduct extends Component {
  ...
  ruleModifierNotPassed = () => {
    try {
      ...
      if (this.state.selectedItem.quantity == 0 || this.state.selectedItem.quantity == undefined) return false;
  ...
  processCart = async () => {
    try {
      ...
      if (selectedItem.mode == 'Add') await this.props.dispatch(OrderAction.processAddCart(defaultOutlet, selectedItem));
      else {
        let response = await this.props.dispatch(OrderAction.processUpdateCart(basket, selectedItem));
  ...
  addItemIsYesNo = async (item, type) => {
    ...
    if (type != 'checkbox') {
      for (let i = 0; i < selectedItem.product.productModifiers.length; i++) {
        if (selectedItem.product.productModifiers[i].modifier.max == 1) {
          for (let j = 0; j < selectedItem.product.productModifiers[i].modifier.details.length; j++) {
            selectedItem.product.productModifiers[i].modifier.details[j].quantity = 0;
            selectedItem.product.productModifiers[i].modifier.details[j].isSelected = false;
          }
        }
      }
    }
    ...
    for (let i = 0; i < selectedItem.product.productModifiers.length; i++) {
      for (let j = 0; j < selectedItem.product.productModifiers[i].modifier.details.length; j++) {
        if (selectedItem.product.productModifiers[i].modifier.details[j].id == item.id) {

          if (selectedItem.product.productModifiers[i].modifier.details[j].quantity == undefined
            || selectedItem.product.productModifiers[i].modifier.details[j].quantity == 0
          ) {
            selectedItem.product.productModifiers[i].modifier.details[j].quantity = 1;
            selectedItem.product.productModifiers[i].modifier.details[j].isSelected = !selectedItem.product.productModifiers[i].modifier.details[j].isSelected;
            selectedItem.product.productModifiers[i].postToServer = true;

          } else {
            selectedItem.product.productModifiers[i].modifier.details[j].quantity = undefined;
            selectedItem.product.productModifiers[i].modifier.details[j].isSelected = !selectedItem.product.productModifiers[i].modifier.details[j].isSelected;
      ...
      for (let j = 0; j < selectedItem.product.productModifiers[i].modifier.details.length; j++) {
        if (selectedItem.product.productModifiers[i].modifier.details[j].id == item.id) {
          if (selectedItem.product.productModifiers[i].modifier.details[j].quantity != undefined
            ...
            return selectedItem.product.productModifiers[i].modifier.details[j].quantity;
          }
        }
      }
    }
        ...
        <div style={{ marginLeft: 5, marginRight: 10 }}>
          {
            item.modifier.details.map(data =>
              <div className="item-modifier">
                <a className="title-modifier" onClick={() => this.addItemIsYesNo(data)}>
                  <input type="checkbox" checked={!data.isSelected} className="scaled-checkbox form-check-input checkbox-modifier" onClick={() => this.addItemIsYesNo(data)} />
  ...
  renderItemCheckbox = (item, i) => {
    return (
      <div className="card card-modifier">
        <div onClick={() => this.toggleModifier(i)} className="card-header header-modifier" style={{ display: 'flex', justifyContent: 'space-between' }}>
          <p className="color" style={{ margin: 0 }}>
            <b>{item.modifierName} <span className="rule-modifier">{this.ruleModifier(item)}</span> </b>
          </p>
        ...
        </div>
        {
          item.modifier.show ?
            <div style={{ marginLeft: 5, marginRight: 10 }}>
              {
                item.modifier.details.map(data =>
                  <div className="item-modifier" className={data.orderingStatus === 'UNAVAILABLE' ? 'item-modifier product-unavailable' : 'item-modifier'}>
                    <a>
                      <input type="checkbox" checked={data.quantity != undefined && data.quantity != 0 && data.orderingStatus != 'UNAVAILABLE' ? true : false} className="scaled-checkbox form-check-input checkbox-modifier" onClick={() => this.addItemIsYesNo(data, 'checkbox')} />
                      <a className="subtitle-modifier">
                        <span className="color">
                          {data.quantity != undefined && data.quantity != 0 ? `${data.quantity}x ` : null}
                        </span>
                        <a onClick={() => this.addItemIsYesNo(data, 'checkbox')}>{data.name}</a>
                        {data.quantity != undefined && data.quantity != 0 ? <span onClick={() => this.selectModifier(data)} data-toggle="modal" data-target="#qty-modifier" className="color" style={{ marginLeft: 5, textDecoration: 'underline' }}>more</span> : null}
                      </a>
                    </a>
                    {
                      data.orderingStatus === 'UNAVAILABLE' ? <p>UNAVAILABLE</p> : <p>{this.getCurrency(data.price)}</p>
                    }
                  </div>
                )
              }
  ...
  ruleModifier = item => {
    try {
      if (
        ...
        item.modifier.min == 1 &&
        (item.modifier.max == 1 || item.modifier.max <= 0 || item.modifier.max == undefined)
      ) {
        return `Pick 1`;
  ...
  toggleModifier = async (i) => {
    let { selectedItem } = this.state;
    selectedItem.product.productModifiers[i].modifier.show = !selectedItem.product.productModifiers[i].modifier.show;
    await this.setState({ selectedItem });
  }
                ...
                item.modifier.details.map(data =>
                  <div className="item-modifier" className={data.orderingStatus === 'UNAVAILABLE' ? 'item-modifier product-unavailable' : 'item-modifier'}>
                    <a className="title-modifier" onClick={() => this.addItemIsYesNo(data)}>
                      ...
                      <input type="radio" checked={data.isSelected ? true : false} class="scaled-checkbox form-check-input checkbox-modifier" onClick={() => this.addItemIsYesNo(data)} />
  ...
  detailProduct = () => {
    ...
    return (
      <div className="modal-content" style={{ width: "100%", height: "90vh", bottom: 0, zIndex: 10 }}>
        ...
        <div className="modal-header modal-header-product" style={{ display: "flex", justifyContent: "center", borderBottom: 'none' }}>
          <button id="close-modal" className="close close-modal" data-dismiss="modal" aria-label="Close" style={{
            position: "absolute", right: 10, top: 14, zIndex: 10, opacity: 0.8, background: '#c00a27', width: 35, height: 35
          }}>
            ...
            !isEmptyObject(selectedItem) ?
              <>
                <div className="col-md-5 col-sm-12" style={{ marginBottom: 15 }}>
                  <center>
                    <img className="detail-image attachment-pizzaro-product-list-fw-col-1 size-pizzaro-product-list-fw-col-1" src={this.renderImageProduct(selectedItem)} />
                  </center>
                </div>
                <div className="col-md-6 col-sm-12">
                  <center>
                    <h3 style={{ textAlign: 'left', marginRight: 10 }} className="color">{selectedItem.product.name}</h3>
                    <p style={{ marginTop: 10, textAlign: 'left', whiteSpace: 'pre-line' }} className="color">{selectedItem.product.description}</p>
                  </center>
                </div>
                <div className="col-md-12">
                  {
                    ...
                    selectedItem.product.productModifiers.map((item, i) => {
                      if (item.modifier.isYesNo !== true && (item.modifier.max == 0 || item.modifier.max === undefined || item.modifier.max > 1 || item.modifier.max == '-')) {
                        return this.renderItemCheckbox(item, i);
                      } else if (item.modifier.isYesNo !== true && item.modifier.max == 1) {
                ...
                </div>
                {
                  defaultOutlet.enableItemSpecialInstructions ?
                    <div className="col-md-12">
                      <label for="name">Special Instruction <span className="text-muted" style={{ fontSize: 10 }}>Optional</span></label>
                      <input
                        onChange={(e) => {
        ...
        <div className="pizzaro-handheld-footer-bar" style={{ display: "flex", justifyContent: "center", backgroundColor: "#FFF", padding: 5, paddingBottom: 20, paddingTop: 20 }}>
          <div className="control-quantity">
            <button disabled={disableButton} style={{ minWidth: 40 }} className="btn btn-increase" onClick={this.decrease}>
              <b style={{ fontSize: 20, color: '#FFF' }}>-</b>
            </button>
            <b className="color" style={{ fontSize: 20, minWidth: 40, textAlign: "center", fontWeight: "bold" }}>{selectedItem.quantity}</b>
            <button disabled={disableButton} style={{ minWidth: 40 }} className="btn btn-increase" onClick={this.increase}>
              <b style={{ fontSize: 20, color: '#FFF' }}>+</b>
            </button>
          </div>
          <div style={{ width: "100%", marginLeft: 10 }}>
            {
              !disableButton ?
                this.isItemExist(selectedItem) ?
                  <button disabled={disableButton} className="btn btn-block btn-footer" onClick={this.processCart}>
                    <b style={{ color: '#FFF' }}>{`${selectedItem.quantity == 0 ? 'Remove' : 'Update'} ${this.calculateTotal()}`}</b>
                  </button> :
                  <button disabled={disableButton} className="btn btn-block btn-footer" onClick={this.processCart}>
  ...
  setOrderingMode = (mode) => {
    localStorage.setItem('webordering_ordering_mode', mode);
    try {
            ...
            <div className="modal-body">
              <div className="col-md-12">
                <div style={{
                  ...
                  {/* {
                    defaultOutlet.enableDineIn || defaultOutlet.enableDineIn == undefined ?
                  ...
                  {/* {
                    defaultOutlet.enableTakeAway || defaultOutlet.enableTakeAway == undefined ?
                  ...
                  {
                    // defaultOutlet.enableDelivery || defaultOutlet.enableDelivery == undefined ?
  ...
  decreaseModifier = async () => {
    let { selectedModifier } = this.state;
    if (selectedModifier.quantity != undefined && selectedModifier.quantity > 0) {
      selectedModifier.quantity -= 1;
      await this.setState({ selectedModifier })
    }
  }
  ...
  modalQtyModifier = () => {
    ...
    return (
      <div className="modal fade" id="qty-modifier" tabIndex={-1} role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            ...
            <div className="modal-body">
              <div className="col-md-12">
                <div className="container" style={{ display: 'flex', width: '100%', justifyContent: 'space-between', alignItems: "center" }}>
                  <button className="btn" style={{ marginLeft: -10, minWidth: 40 }} onClick={this.decreaseModifier}>
                    <b style={{ fontSize: 20 }}>-</b>
                  </button>
                  <div><b style={{ fontSize: 20 }}>{selectedModifier.quantity}</b></div>
                  <button className="btn" style={{ marginRight: -10, minWidth: 40 }} onClick={this.increaseModifier}>
                    <b style={{ fontSize: 20 }}>+</b>
                  </button>
                </div>
                {
                  selectedModifier.quantity == 0 ?
                    <button id="dismiss-ordering-mode" data-dismiss="modal" className="btn btn-block btn-danger"
                      style={{ marginTop: 30, marginBottom: 20, height: 40, fontWeight: "bold" }}>
                      Remove
                    </button> :
                    <button id="dismiss-ordering-mode" data-dismiss="modal" className="btn btn-block btn-footer"
                      style={{ marginTop: 30, marginBottom: 20, height: 40, fontWeight: "bold" }}>
                      OK
                    </button>
                }
  ...
  modalMessage = () => {
    ...
    return (
      <div className="modal fade" id="mesage-modifier" tabIndex={-1} role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div className="modal-dialog modal-dialog-product modal-dialog-centered modal-full" role="document" style={{ justifyContent: 'center' }}>
          <div className="modal-content" style={{ width: 340 }}>
            <div className="modal-header" style={{ display: "flex", justifyContent: "center" }}>
              <h5 className="modal-title" style={{ fontSize: 20 }}>{message.title}</h5>
              <button type="button" id="btn-close-mesage-modifier" className="close" data-dismiss="modal" aria-label="Close" style={{
  ...
  render() {
    ...
    return (
      <div>
        <span data-toggle="modal" data-target="#mesage-modifier" id="btn-mesage-modifier" />
        <div className="modal fade bd-example-modal-lg" id="detail-product-modal" tabIndex={-1} role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
