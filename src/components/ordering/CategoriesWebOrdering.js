...
export default class Categories extends Component {
      ...
      this.props.categories.forEach((element, i) => {
        try {
          ...
          if (target.offsetTop <= (window.pageYOffset + 200) && (target.offsetTop + 110) >= window.pageYOffset) {
            ...
            try {
              document.getElementById(`cat-${i}`).scrollIntoView({ behavior: "smooth", inline: "center" });
            } catch (e) { }
          }
        } catch (e) { }
      })
  ...
  render() {
    ...
    return (
      <ul id="header-categories" className="nav nav-tabs pizzaro-nav-tabs categories-product relative-position" style={{ marginBottom: 0 }}>
        {
          !openSearch ?
            this.props.categories.map((item, i) =>
              ...
              >
                ...
                {
                  i == selectedCategory &&
                  <div className="profile-dashboard" style={{ height: 4, marginLeft: -10, position: "absolute", bottom: -6, zIndex: 10, width: "100%" }} />
                }
              </li>
            )
            :
            <input
              onKeyUp={(e) => { this.isItemsFinishedToLoad(e.target.value); }}
        ...
        <div className="search-button-absolute" id="search-button-category" style={{ height: 40, marginTop: -11, width: 40 }}>
          {
            openSearch ?
              <i
