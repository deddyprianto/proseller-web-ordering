...
class Ordering extends Component {
  ...
  getProductPreset = async (categories, outlet) => {
    ...
    while (i < categories.length && this.state.processing) {
      ...
      let data = await this.props.dispatch(ProductAction.fetchProduct(categories[i], outlet, 0, 5));
      let items = {
      ...
      if (data.dataLength > 0) {
        ...
        while (j <= data.dataLength && this.state.processing) {
          let product = await this.props.dispatch(ProductAction.fetchProduct(categories[i], outlet, j, 5))
  ...
  selectProduct = async (productSelected, mode) => {
    ...
    if (isEmptyObject(basket)) {
      ...
      if (!isEmptyArray(basket.details)) {
        const find = await basket.details.find(data => data.product.id == product.product.id);
        if (find != undefined) {
  ...
  render() {
    ...
    return (
      <div className="section-tabs container-product" data-toggle="modal" data-target="#modal-product">
        ...
        <span data-toggle="modal" data-target="#ordering-mode" id="open-modal-ordering-mode" style={{ color: 'white' }}></span>
      </div>
    );
  }
}
