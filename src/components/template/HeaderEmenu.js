...
class Header extends Component {
  ...
  componentDidMount = async () => {
    ...
    if (isLoggedIn) {
      let response = await this.props.dispatch(InboxAction.getBroadcast({ take: 14, skip: 0 }));
  ...
  getShareURL = async () => {
    ...
    if (isEmptyObject(basket)) {
      ...
      const response = await this.props.dispatch(OrderAction.shareURL(1, basket.outletID, 'DINEIN'));
      if (response != false) {
        let URL = config.url_crm.replace('crm/api/', 'emenu?order=');
        URL += response.data.accessToken;
        URL = encodeURI(URL);

        await this.setState({ shareURL: `Hey friend, follow below link for sharing order ${URL}` });
  ...
  searchProduct = async (query) => {
    try {
      let productsBackup = encryptor.decrypt(JSON.parse(localStorage.getItem(`${config.prefix}_productsBackup`)));
      ...
      for (let i = 0; i < productsBackup.length; i++) {
        ...
        try {
          for (let j = 0; j < productsBackup[i].items.length; j++) {
            if (productsBackup[i].items[j].product.name.toLowerCase().includes(query.toLowerCase())) {
  ...
  render() {
    ...
    return (
      <div id="header-cwo">
        <LoginRegister />
        <span data-toggle="modal" data-target="#login-register-modal" id="login-register-btn" />
        <header id="masthead" className="site-header header-v4 lite-bg" style={{ position: "fixed", width: "100%", padding: '30px 20px', boxShadow: "1px 2px 5px rgba(128, 128, 128, 0.5)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginLeft: 5, marginRight: 5, marginTop: -10, marginBottom: -15 }}>
            {
              !openSearch ?
                <div style={{ display: "flex", }}>
                  <Link to="/">
                    <a className="custom-logo-link" rel="home">
                      <img src={infoCompany.imageURL || config.url_logo} style={{ height: 30, objectFit: "contain" }} />
                    </a>
                  </Link>
                  <div style={{ marginLeft: 5, fontWeight: "bold" }}>
                    <LocationOnIcon className="color" style={{ fontSize: 25, marginBottom: -5 }} />
                    <span className="color" style={{ fontSize: 16 }}>{defaultOutlet.name && defaultOutlet.name.substring(0, 8)}</span>
                  </div>
                </div> :
                <input
            ...
            <div>
              ...
              }
              {
                ...
                <i onClick={() => {
                  ...
                  setTimeout(() => { document.getElementById('input-txt').classList.add('active') }, 100)
                }}
                  style={{ fontSize: 23, cursor: 'pointer' }}
                ...
                <Link to="/inbox">
                  {
                    ...
                    <div style={{
                      ...
                      width: 20, borderRadius: 20, height: 20, display: "flex", color: "#FFF",
                      alignItems: "center", justifyContent: "center", top: 13, right: 15,
                      border: "2px solid #FFF", fontWeight: "bold"
                    }}>{broadcastUnreadLength}</div>
                  }
                  <i style={{ fontSize: 23, cursor: 'pointer', marginLeft: 20 }}
