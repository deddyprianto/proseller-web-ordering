      ...
      await product.product.productModifiers.map((group, i) => {
        if (!isEmptyArray(group.modifier.details))
          group.modifier.details.map((detail, j) => {
            ...
            if (group.modifier.isYesNo == true && detail.orderingStatus == 'AVAILABLE') {
              if (group.modifier.yesNoDefaultValue == true && detail.yesNoValue == 'no') {
                product.product.productModifiers[i].modifier.details[j].isSelected = false;
              }

              if (group.modifier.yesNoDefaultValue == false && detail.yesNoValue == 'yes') {
        ...
        />
        {
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14 }}>
              <div style={{ fontWeight: "bold", color: "#20a8d8" }}> {data.storeDetail.name} </div>
              <Link to="/">
                <div style={{
                  fontWeight: "bold", cursor: "pointer", color: "#20a8d8",
                  border: "1px solid #20a8d8", borderRadius: 5, width: 100,
            ...
            <div style={{ backgroundColor: "#DCDCDC", height: 3, marginBottom: 10, marginTop: 10 }} />
            <div style={{ display: "flex", justifyContent: 'space-between', fontSize: 14, marginTop: 5 }}>
              <div style={{ display: "flex", marginLeft: -8, alignItems: "center" }}>
                <input type="checkbox" style={{ marginTop: -1 }} checked={selected === dataBasket.details.length}
              ...
              <div onClick={() => !this.props.roleBtnClear && this.props.handleClear(dataBasket)} style={{
                fontWeight: "bold", cursor: "pointer", color: "#20a8d8",
                border: "1px solid #20a8d8", borderRadius: 5, width: 100,
                display: "flex", alignItems: "center", justifyContent: "center",
            ...
            <div style={{ backgroundColor: "#DCDCDC", height: 3, marginBottom: 10, marginTop: 10 }} />
            {
              dataBasket.details.map((item, key) => (
                <div>
                  <div style={{ display: "flex", marginLeft: -8, alignItems: "center" }}>
                    <input type="checkbox" style={{ marginTop: -1, marginRight: 10 }} checked={item.selected !== false}
                      onClick={() => this.handleSelect(key, item)}
                      className="scaled-checkbox form-check-input checkbox-modifier" />
                    <CardItemBasket key={key}
                      data={item}
                  ...
                  <div style={{ backgroundColor: "#DCDCDC", height: 1, marginBottom: 10, marginTop: 10 }} />
                </div>
              ))
            }
            <span data-toggle="modal" data-target="#detail-product-modal" id="detail-product-btn" />
          </div>
        }
