...
class EditProfile extends Component {
  ...
  validationField = (field, value) => {
    ...
    this.props.fields && this.props.fields.find(items => { 
      if(items.children) {
        let check = items.children.find(child => { return child.fieldName.toLowerCase() === field.toLowerCase()})
  ...
  handleUpdateProfile = async () => {
    ...
    if (this.state.defaultEdit.name) payload.newName = this.state.dataCustomer.name;
    if (this.state.defaultEdit.phoneNumber) payload.newPhoneNumber = this.state.dataCustomer.phoneNumber;
    if (this.state.defaultEdit.email) payload.newEmail = this.state.dataCustomer.email;
    ...
    if(listName !== ""){
      ...
      Swal.fire({ icon: "warning", timer: 1500, title: `${listName} is required`, showConfirmButton: false, });
      return
    }
  ...
  viewPassword(){
    let { defaultError, showOldPassword, showNewPassword, showRePassword } = this.state;
            ...
            <main id="main" className="site-main" style={{ width: "100%"}}>
              ...
              }
              {
                ...
                <div style={{marginTop: -20 }}>
                  ...
                  <Button
                    className="button"
                    style={{ width: "100%", marginTop: 20, borderRadius: 5, height: 50, }}
